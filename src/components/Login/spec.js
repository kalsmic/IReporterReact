...
describe('Login Component', () => {
  ...
  it('should handle the onchange event', () => {
    ...
    wrapper.instance()
      .handleChange(event);
    expect(wrapper.instance().state.username)
      .toBe('kalsmic');
  });
  ...
  it('should handle the onSubmit event', () => {
    ...
    expect(wrapper.instance().state.username)
      .toBe('');
    expect(wrapper.instance().state.password)
      .toBe('');
    expect(instance.props.authenticateUser)
      .not
      .toBeCalled();

    wrapper.setState({
      username: 'admin',
      password: 'Pa4$word123'
    });
  ...
  it('should show messages to the user', () => {
    ...
    expect(wrapper.find('.success-msg')
      .text())
